...
Mac 客户端的默认下载路径为： ~/Movies/bilibili/ (即 Finder 中的 电影/bilibili)
```
1476846399/                 # 视频 ID 文件夹 (直接拖入此目录)
├── 1476846399-1-30120.m4s  # 视频流 (DASH 编码)
├── 1476846399-1-30280.m4s  # 音频流 (DASH 编码)
├── videoInfo.json          # 关键元数据（包含标题、分区、UP主信息）
├── group.jpg               # 视频封面图
└── view                    # 缓存索引文件
```
#### 第二步：设置输出目录
